...
import com.sciome.bmdexpress2.service.DuckDBExportServiceV3;

/**
 * Command-line runner for DuckDB Export Service V3 - CamelCase Schema
 * 
 * Usage: java -cp 'target/classes:lib/*' com.sciome.bmdexpress2.commandline.DuckDBExportRunnerV3 input.bm2 output.duckdb
 */
public class DuckDBExportRunnerV3 {
    
    public static void main(String[] args) {
        if (args.length != 2) {
            System.err.println("Usage: java DuckDBExportRunnerV3 <input.bm2> <output.duckdb>");
            System.exit(1);
        }
        ...
        try {
            System.out.println("DuckDB Export Service V3 - CamelCase Schema");
            System.out.println("===========================================");
            ...
            // Export to DuckDB
            System.out.println("\nExporting to camelCase DuckDB schema...");
            DuckDBExportServiceV3 exportService = new DuckDBExportServiceV3();
            ...
            if (dbFile.exists()) {
                ...
                System.out.println("  SELECT * FROM categoryAnalysisResults WHERE geneAllCount >= 40 AND geneAllCount <= 500 AND percentage >= 5;");
                System.out.println("  SELECT categoryIdentifierId, geneAllCount, genesThatPassedAllFilters, bmdFifthPercentileTotalGenes FROM categoryAnalysisResults;");
            } else {
