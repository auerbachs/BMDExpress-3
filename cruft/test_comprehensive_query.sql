...
probe_responses_data AS (
  SELECT
    dre.id as experiment_id,
    json_group_array(
      json_object(
        'id', pr.id,
        'doseResponseExperimentId', pr.doseResponseExperimentId,
        'probeId', pr.probeId,
        'responses', pr.responses
      )
    ) as probeResponses
  FROM doseResponseExperiments dre
  JOIN probeResponses pr ON dre.id = pr.doseResponseExperimentId
  GROUP BY dre.id
),
treatments_data AS (
  SELECT
...
SELECT
  ...
  json_object(
    ...
    'probeResponses', COALESCE(prd.probeResponses, '[]'),
...
LEFT JOIN probe_responses_data prd ON cad.experiment_id = prd.experiment_id
