...
public class BMDStatatistics
{
			...
			CategoryAnalysisResult categoryAnalysisResult, Hashtable<String, Vector> subHashG2Ids)
	{
		...
		List<BMDRange> bmdRangeList = new ArrayList<>();
		for (int i = 0; i < sub; i++)
		{
			...
			if (subHashG2Ids.get(gene) != null)
			{
				...
				BMDRange range = new BMDRange();
				range.bmd = perGBMDs[i].avgBmd();
				range.bmdl = perGBMDs[i].avgBmdl();
				range.bmdu = perGBMDs[i].avgBmdu();
				bmdRangeList.add(range);
			}
		}
		...
		// create sortbmd/l/u's such that all is sorted relative to bmd
		bmdRangeList.sort((c1, c2) ->
		{
			return c1.bmd.compareTo(c2.bmd);
		});

		double sortBmds1[] = new double[bmdRangeList.size()];
		double sortBmdls1[] = new double[bmdRangeList.size()];
		double sortBmdus1[] = new double[bmdRangeList.size()];
		for (int i = 0; i < bmdRangeList.size(); i++)
		{
			sortBmds1[i] = bmdRangeList.get(i).bmd.doubleValue();
			sortBmdls1[i] = bmdRangeList.get(i).bmdl.doubleValue();
			sortBmdus1[i] = bmdRangeList.get(i).bmdu.doubleValue();
		}

		col = percentBMD(col, all, 0.05, sortBmds1, sortBmdls1, sortBmdus1, categoryAnalysisResult, true);
		col = percentBMD(col, all, 0.1, sortBmds1, sortBmdls1, sortBmdus1, categoryAnalysisResult, false);
			...
			if (pct >= percent)
			{
				...
				else
				{
					...
					else
					{
						...
						categoryAnalysisResult.setBmdlTenthPercentileTotalGenes(avgl);
						categoryAnalysisResult.setBmduTenthPercentileTotalGenes(avgu);
					}
	...
	public Vector<String> checkABSModelFCAbove(Vector<String> vectGenes, double absModelFC,
			Hashtable<String, Vector> subHashG2Ids, Set<String> removedProbes)
	{
		Vector<String> pcGenes = new Vector<String>();
			...
			for (int i = vectGenes.size() - 1; i >= 0; i--)
			{
				...
				if (probes != null && probes.size() > 0)
				{
					for (int j = probes.size() - 1; j >= 0; j--)
					{
						...
						if (probeStatResult.getBestStatResult() == null
								|| !Double
										.isFinite(probeStatResult.getBestStatResult().getAbsFoldChangeToTop())
								|| probeStatResult.getBestStatResult().getAbsFoldChangeToTop() < absModelFC)
						{
							probes.remove(st);
							removedProbes.add(st);
						}
	...
	public Vector<String> checkABSZScoreAbove(Vector<String> vectGenes, double absZScore,
			Hashtable<String, Vector> subHashG2Ids, Set<String> removedProbes)
	{
		Vector<String> pcGenes = new Vector<String>();
				...
				if (probes != null && probes.size() > 0)
				{
					for (int j = probes.size() - 1; j >= 0; j--)
					{
						...
						if (probeStatResult.getBestStatResult() == null
								|| !Double
										.isFinite(probeStatResult.getBestStatResult().getAbsFoldChangeToTop())
								|| probeStatResult.getBestStatResult().getAbsZScore() < absZScore)
						{
							probes.remove(st);
							removedProbes.add(st);
						}
	...
	public Vector<String> checkStepFunction(Vector<String> vectGenes, Hashtable<String, Vector> subHashG2Ids,
			Set<String> removedProbes)
	{
		Vector<String> pcGenes = new Vector<String>();
